...
Database::Database()
    : m_Sqlite(Wt::WApplication::instance()->appRoot() + "usernamedb.db") /*not sure if i can make this scalable by splitting the username a-z thing in half... since it's so tied in with Wt. but a custom rewrite of the register/login/etc functionality may not be too much of a stretch at that point*/
{
...
void Database::GlobalConfigure()
{
    ...
    s_AuthService.setAuthTokensEnabled(true, "anonymousbitcoincomputing");
