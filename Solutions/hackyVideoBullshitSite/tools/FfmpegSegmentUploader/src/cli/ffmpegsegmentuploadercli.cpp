...
{
    handleD("Usage:");
    handleD("FfmpegSegmentUploaderCli filenameOfSegmentsEntryList localPathWhereSegmentsAreSaved remoteDestinationToUploadTo remoteDestinationToMoveTo userHostPathComboSftpArg [sftpProcessPath=/usr/bin/sftp]");
}
void FfmpegSegmentUploaderCli::cliUserInterfaceMenu()
{
    handleD("Available Actions (H to show this again):");
    handleD("\tq\t- Quit"); //TODOreq: decide if "quit now", "quit after current upload finishes", or "quit when queue is next seen empty", or all three :-P. the last one implies that ffmpeg is stopped separately/before
}
void FfmpegSegmentUploaderCli::handleStandardInputReceived(const QString &standardInputLine)
{
    QString standardInputLineToLower = standardInputLine.trimmed().toLower();
    ...
    {
        handleD("stopping ffmpeg segments uploader...");
