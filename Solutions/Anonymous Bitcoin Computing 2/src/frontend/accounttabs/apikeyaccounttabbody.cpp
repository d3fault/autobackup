...
void ApiKeyAccountTabBody::displayApiKey(const string &couchbaseDocument, bool lcbOpSuccess, bool dbError)
{
    ...
    new WBreak(this);
    new WBreak(this);
    WAnchor *simplePhpApiClientAnchor = new WAnchor(WLink(WLink::Url, ABC_API_SIMPLE_PHP_CLIENT_URL), "Simple PHP API Client", this);
    ...
    new WBreak(this);
    new WBreak(this);
    new WText("General Instructions (FOR PROGRAMMERS): Your website polls the API once every 5 minutes when the ad campaign has no purchases. When someone buys your ad, the relevant hovert/alt text, URL, and ad image (all 3 are base64 encoded) is supplied in a json document. Additionally, the ad's expiration unix timestamp is supplied as an integer (You do not query the API again until the ad expires). Lastly, there is a json field called \"error\". If there is no error, it's value is \"none\". If there is an error, it gives a description of the error. Note that when the ad space is not purchased there is an \"error\" of \"" ABC2_API_AD_SLOT_NOT_PURCHASED_ERROR "\", in which case you can display anything or nothing (we recommended you at least say 'click here to buy this ad space'). The 'Simple PHP API Client' provided above also doubles as a working exampke. It shows how a 'no.ad.placeholder.jpg'' image is used whenever \"error\" is not \"none\". You must cache/save the results of your API query and SERVE THE AD IMAGE TO YOUR END USERS YOURSELF, and the 'Simple PHP API Client' demonstrates this as well. You may only query the API to get the details for a purchased ad " GetTodaysAdSlotServerClientConnection_MAX_API_REQUESTS_PER_AD_SLOT_DURATION_EXCLUDING_NO_AD_RESPONSES_OFC_STR " times. Querying more times than that will result in an error until the next ad begins (Note: when the ad slot is not currently purchased, you can query the API unlimited times, but only once every 5 minutes).", this);
    new WBreak(this);
    new WBreak(this);
    new WText("Example API Query (HTTPS): https://anonymousbitcoincomputing.com:" + s_ApiHttpsPort + ABC2_API_GET_CURRENT_AD_ACTION_FULL_PATH_MACRO(m_AbcApp->m_CurrentlyLoggedInUsername, "0", apiKey), this);
