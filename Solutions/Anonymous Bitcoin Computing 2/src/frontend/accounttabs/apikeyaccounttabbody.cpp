...
std::string ApiKeyAccountTabBody::m_ApiHttpsPort;
...
void ApiKeyAccountTabBody::displayApiKey(const string &couchbaseDocument, bool lcbOpSuccess, bool dbError)
{
    ...
    new WBreak(this);
    new WBreak(this);
    new WText("General Instructions: If you have no programming experience, the simplest way to get purchased ads showing up on your website is by placing an HTML snippet on your site (wherever you want the ad to be displayed), and additionally placing a PHP file in the same directory as that HTML file. If you don't have PHP installed on your server (or if you don't know if you have PHP installed on your server), contact your IT administrator for help." /*i love that empty statement, "contact your IT administrator". It's code for "you are a fucking noob, I'm not explaining this to you"*/, this); //TODOoptional: link to a my profile on some freelance site. DO-NOT-TODO-YET(I'd an hero): code a freelance site (use abc code as starting point tbh)
    new WBreak(this);
    new WBreak(this);
    new WBreak(this);
    new WText("General Instructions (FOR PROGRAMMERS): Your website polls our API once every 5 minutes when the ad campaign has no purchases. When someone buys your ad, the relevant hovert/alt text, URL, and ad image (all 3 are base64 encoded) is supplied in a json document. Additionally, the ad's expiration unix timestamp is supplied as an integer. Lastly, there is a json field called \"error\". If there is no error, it's value is \"none\". If there is an error, it gives a description of the error. Note that when the ad space is not purchased there is an \"error\" of \"" ABC2_API_AD_SLOT_NOT_PURCHASED_ERROR "\", in which case you can display anything or nothing (we recommended you at least say 'click here to buy this ad space'). Check the example to see how a no.ad.placeholder.jpg image is used whenever \"error\" is not \"none\". You must cache/save the results of your API query and SERVE THE AD IMAGE TO YOUR END USERS YOURSELF (again, the provided example demonstrates this). You may only query the API to get the details for a purchased ad " GetTodaysAdSlotServerClientConnection_MAX_API_REQUESTS_PER_AD_SLOT_DURATION_EXCLUDING_NO_AD_RESPONSES_OFC_STR " times. Querying more times than that will result in an error until the next ad begins (Note: when the ad slot is not currently purchased, you can query the API unlimited times, but only once every 5 minutes).", this);
    new WBreak(this);
    new WBreak(this);
    new WText("Example API Query (HTTPS): https://anonymousbitcoincomputing.com:" + m_ApiHttpsPort + ABC2_API_GET_CURRENT_AD_ACTION_FULL_PATH_MACRO(m_AbcApp->m_CurrentlyLoggedInUsername, "0", apiKey), this);
    //TODOreq: new WBreak(this); new WText("Example API Query (Tor Hidden Service): abcapi0983409283.onion etc -- don't need port because it will 'appear' to be port 80 i think...");
}
