...
CONFIGURE
1) Open the file named 'abc.api.generate.current.ad.html.php' in notepad, or your text editor of choice. At the top you'll see a section called 'MANDATORY CONFIG'. Fill in these 6 necessary values
...
5) Where it says 'Replace-With-Your-Ad-Image-Width', you'll fill in the width of the ad image that you specified when setting up the ad campaign
6) Where it says 'Replace-With-Your-Ad-Image-Height', you'll fill in the height of the ad image that you specified when setting up the ad campaign
7) The last option, already filled in with 'no.ad.placeholder.jpg', indicates what image will be shown whenever the ad space has not been purchased. You should create an image to be displayed in that case, and supply it's filename (or name your custom image 'no.ad.placeholder.jpg') in the config. The image file needs to be publically accessible on your web server, just like the PHP file mentioned in step (2) of INSTALL
...
3) Paste the following HTML snippet into whatever HTML file you want the ad to be displayed in (or wherever you can paste an HTML snippet, if you're using a content management system such as Wordpress). In the HTML snippet towards the bottom, you'll see a width and height (576 and 96, respectively). You need to update those values with the same values you set in steps (5) and (6) of CONFIGURE, otherwise the ad image will not display properly for clients without javascript enabled
...
<noscript>
	<iframe src='abc.api.generate.current.ad.html.php?foriframe=1' width='576' height='96' frameborder='0' marginwidth='0' scrolling='no'></iframe>
</noscript>
<-- END ABC PHP API CLIENT HTML SNIPPET -->

4) The included example.index.html file demonstrates the above HTML snippet pasted into a correct place
5) Save the HTML file or submit the snippet or whatever, upload it to your webserver if you need to, and then go visit the site. If nobody has purchased your ad space, you should see the 'no ad placeholder' image you created in step (7) of CONFIGURE. Once someone purchases your ad space, it will take up to ten minutes for the advertisement to be displayed (and it will only show up after a page refresh)
