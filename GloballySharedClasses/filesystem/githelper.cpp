...
recursively set all modes of every file for every commit (so git doesn't record them changing)
fix all exitCodes areas to dump string results before returning false
"nothing to commit (working directory clean)" does not return exit code 0 (wtf), handle appropriately -- keep track of how many times it happens so you can subtract it from the "preCensorCommitCount" <--> "postCensorCommitCount" comparison :)
^^^^^A COMMIT LOG MESSAGE (just 1) IS LOST :(
^^^^^^^^Just an "Auto-Commit", fuck it :)...
*/
...
bool GitHelper::gitLogReturningCommitIdsOnly(QList<GitCommitIdTimestampAndMessage*> *commitIdsStringListToFill, QString semiOptionalDirToRunGitLogIn)
{
    ...
    gitLogArguments << "log";
    gitLogArguments << "--format=format:%H%n%aD%n%s"; //woot fuck parsing <3 linus (not that this is any indication of his greatness (btw OpenBSD is bettar and I would be honored if he called me a masturbating monkey :-D. That's what life is all about~))
