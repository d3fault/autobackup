...
MultiServerAbstraction::MultiServerAbstraction(IMultiServerBusiness *serverBusiness, QObject *parent)
    : QObject(parent), m_ServerBusiness(serverBusiness), m_SslTcpServer(0) /* etc tcp local */
{
...
void MultiServerAbstraction::handleNewClientSentData()
{
    ...
    if(newClient)
    {
        ...
        if(serverHelloStatus)
        {
            ...
            m_NetworkStreamToClient.setDevice(newClient);

            while(!m_NetworkStreamToClient.atEnd())
            {
                ...
                m_TheMessage.clear(); //TODOreq: is this necessary or does "streaming into" it overwrite previous contents?
                m_NetworkStreamToClient >> m_TheMessage; //woo. consume the message. we _know_ we have enough bytes!
                serverHelloStatus->m_NetworkMagic.messageHasBeenConsumedSoPlzResetMagic();

                if(!m_TheMessage.isNull() && !m_TheMessage.isEmpty())
                {
                    ...
                    QDataStream messageReadstream(&m_TheMessage, QIODevice::ReadOnly);
                    ...
                    switch(currentState)
                    {
                        ...
                        case ServerHelloStatus::DoneHelloing: //optimize for DoneHelloing case, as it will be used the most times over and over and over and over and over during production
                        {
                            m_ServerBusiness->messageReceived(&m_TheMessage, serverHelloStatus->cookie());
                        }
                        break;
                        case ServerHelloStatus::AwaitingHello:
                        {
                            ...
                            NetworkMagic::streamOutMagic(&m_NetworkStreamToClient);
                            m_NetworkStreamToClient << welcomeMessage;
                        ...
                        case ServerHelloStatus::HelloReceivedWelcomeDispatchedAwaitingThankYouForWelcomingMe:
                        {
                            ...
                            if(cookieForConfirming == serverHelloStatus->cookie())
                            {
                                ...
                                NetworkMagic::streamOutMagic(&m_NetworkStreamToClient);
                                m_NetworkStreamToClient << okStartSendingMessagesBroMessage; //TODOreq: i am forgetting magic for the client in my responses!!!
                            }
                            else
                            {
