...
//TODOreq (i think this might be per-connection now too and therefore thread safe, but idfk): this is NOT thread safe, since they all use the same byte array and data stream [as an optimization]. Magic _IS_ thread-safe because each connection (and therefore thread) has it's own NetworkMagic object. This is just a peek helper utility so it's a bit different in that respect
