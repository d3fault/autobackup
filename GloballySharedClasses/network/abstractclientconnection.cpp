        ...
        //set up flagging for
         ...
         if(queueActionResponsesBecauseTheyMightBeReRequestedInNewConnection)
         {
            //build a hash of all the ones in business pending RIGHT THIS VERY MOMENT because those ones are difficult to ensure they go back only when appropriate. specifically, if they are in business pending for the entire duration of the merge, they won't notice anything happened if they only depend on the flag above. this list ensures us that all the ones in business pending will wait for a retry -- TODOreq i think that functionality is only half implemented (the lists are built in the notify call below)
             ...
             m_ProtocolKnower->notifyThatQueueActionResponsesHasBeenEnabled();
         }
    }
}
...
void AbstractClientConnection::makeConnectionBad()
{
    ...
    //{
        ...
        m_MultiServerAbstraction->appendDeadConnectionThatMightBePickedUpLater(this); //TODOreq: remove (deleting members/children appropriately) where appropriate and possibly also remove after a certain amount of time so the list doesn't get too big

        setQueueActionResponsesBecauseTheyMightBeReRequestedInNewConnection(true);
        //TODOreq: after enabling queue mode here, our next entry point would be when the same cookie is received. however i know that that code also sets queue mode. i need to make sure that it doesn't "build the list twice" etc (but i think the underlying setQueue has the proper code for that (it still seems sloppy to set it to true when it's already true)... making sure the variable is actually changing before doing ANYTHING)
