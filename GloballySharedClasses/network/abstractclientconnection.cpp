...
void AbstractClientConnection::setQueueActionResponsesBecauseTheyMightBeReRequestedInNewConnection(bool queueActionResponsesBecauseTheyMightBeReRequestedInNewConnection)
{
    if(queueActionResponsesBecauseTheyMightBeReRequestedInNewConnection != m_QueueActionResponsesBecauseTheyMightBeReRequestedInNewConnection)
    {
        //set up flagging for
         ...
         {
            //build a hash of all the ones in business pending RIGHT THIS VERY MOMENT because those ones are difficult to ensure they go back only when appropriate. specifically, if they are in business pending for the entire duration of the merge, they won't notice anything happened if they only depend on the flag above. this list ensures us that all the ones in business pending will wait for a retry -- TODOreq (pretty sure done) i think that functionality is only half implemented (the lists are built in the notify call below). by half implemented i may have meant that they never get... re-requested by the next connection, and possibly/probably never purged after a timeout
             //TO DOnereqNOWMOTHERFUCKER: determine if you want to use retry (message contents included) or a status ability. should the message retry also be a status poll if. ok fuck merging toggleBit with retryBit or anything with anything. if I'm struggling with it, I'm not KISS. so just use the flags for their names and yea that is the decision. if you add status (like a special-case-response) capabilities to the first retry (and probably the second too), that is fine i guess?
...
void AbstractClientConnection::mergeNewIoDevice(QIODevice *newIoDeviceToClient)
{
    ...
    m_IoDeviceToClient->deleteLater(); //TODOreq: take out of lists first??? might already be done but no clue tbh. null pointer exceptions if we don't! i think i also need to handle the case where i take it out of the dead connections list right around here too (or at least somewhere!)
    ...
    //^^being handled, yes.. but done? idfk. might only be halfway done (setting up the lists of those special in business messages)
    setQueueActionResponsesBecauseTheyMightBeReRequestedInNewConnection(false); //TODOreq:^^^^
}
...
void AbstractClientConnection::handleDataReceivedFromClient()
{
    ...
    while(!m_DataStreamToClient.atEnd())
    {
        ...
        if(!m_ReceivedMessageByteArray.isNull() && !m_ReceivedMessageByteArray.isEmpty())
        {
            ...
            //TO DOnereq: read in the hello state from the message instead of relying on an implied member state
            ...
            switch(helloStateFromClientInMessage)
            {
                ...
                case InitialHelloFromClient:
                {
                    ...
                    if(containsCookie)
                    {
                        ...
                        //TODOreq: old: eh i guess at some point (later on, but still has to do with hello protocol) i should trigger the sending of all their queued messages.
                        //^^NO, THEY MUST BE RE-REQUESTED
                        ...
                        m_OldConnectionToMergeOnto = m_MultiServerAbstraction->potentialMergeCaseAsCookieIsSupplied_returning_oldConnection_ifMerge_or_ZERO_otherwise(this); //TODOreq: for the socket error -> reconnect with same cookie use case, we might find that the old connection isn't in the list that is scanned (as it may have been taken out when socket error was detected). fix that to make it work
                    }
                ...
                case ThankYouForWelcomingMeFromClient:
                {
                    ...
                    if(cookieForConfirming == cookie())
                    {
                        ...
                        if(m_OldConnectionToMergeOnto)
                        {
                            ...
                            //the problem is that I have 2 abstractClientConnections and 2 protocolKnowers and 2 ioDevices, when I only want one of each. But I want to take/grab all of the "pending ack" and also receive "business pending" (TODOreq (done maybe no clue)) that complete soon and send them over the NEW io device. the old io device is in queue mode at this moment so it doesn't even attempt to send, just adds to the pending ack list
                            //i think it would be simpler to delete the new 'abstractclientconnection' and 'protocol knower' and (after safely un-childing) moving the iodevice to to the old pair. the old protocol knower already has signals in the business pointed at it's slots, so it's definitely easier that way. TODOreq (done maybe idk): might need to explicitly delete the iodevice if i DON'T have it as a child of [whichever of the pair own it, i forget]. Sweet just checked, the io device doesn't have a parent so moving it around is easy. That 'this' and/or 'protocolknower' are parented is irrelevant, can just deleteLater them once the io device is moved.
...
void AbstractClientConnection::makeConnectionBad()
{
    ...
    //{
        ...
        //TODOreq: doesn't belong here but i don't think my code correctly reacts to non-fatal socket errors (ssl errors for example). i should perhaps use m_ConnectionBad after all, but maybe should use it at the very beginning of a socket READ (i don't think i need it for the socket write as the queue mode bool should be checked before attempting that, but we COULD and it wouldn't slow us down that much). whatever is connected to that readyRead should perhaps check if the connection is still good. you might think that queue mode being enabled etc means that slot will never be called, but we can't make any guarantees for what the client must do and must be flexible! we could also put it into a different "state" as a way of dodging it, but it doesn't matter 'how' you do it as long as you do it

        m_MultiServerAbstraction->dontBroadcastTo(this); //TODOreq: mentioned a few times before but important that when the next connection with our same cookie is received, we still need to be able to "find" it as our "old connection to merge onto". this call to dontBroadcastTo messes that up because it removes us from m_ListOfHellodConnections. maybe we can just use a special list for this case. insta-merges (dupe cookie seen first) don't need it because they call dontBroadcastTo after the search for the old one (they find it and call it on it!), but this socket error case will put us in a "maybe this connection will be picked up later but we have no idea [yet]" mode (which doesn't apply to insta-merge because we already know that yes it is being picked up)

        m_MultiServerAbstraction->appendDeadConnectionThatMightBePickedUpLater(this); //TODOreq: remove (deleting members/children appropriately) where appropriate (new connection with matching cookie, replacing this one) and possibly also remove after a certain amount of time so the list doesn't get too big

        setQueueActionResponsesBecauseTheyMightBeReRequestedInNewConnection(true);
        //TODOreq (pretty sure it's done, AT LEAST HACKILY it is): after enabling queue mode here, our next entry point would be when the same cookie is received. however i know that that code also sets queue mode. i need to make sure that it doesn't "build the list twice" etc (but i think the underlying setQueue has the proper code for that (it still seems sloppy to set it to true when it's already true)... making sure the variable is actually changing before doing ANYTHING)
        //^^^pretty sure this is good enough but going to leave it here just to be safe
    //}
}
