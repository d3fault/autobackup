...
bool ByteArrayMessageSizePeekerForIODevice::enoughBytesAreAvailableToReadTheByteArrayMessage(QIODevice *ioDeviceToPeekByteArrayMessageSizeOn)
{
    if(ioDeviceToPeekByteArrayMessageSizeOn->bytesAvailable() < sizeof(quint32))
    {
        return false;
    }
    ...
    if(m_PeekedByteArraySizeRaw.size() != sizeof(quint32))
    {
    ...
    if((ioDeviceToPeekByteArrayMessageSizeOn->bytesAvailable()-sizeof(quint32)) < peekedByteArraySize) //we check to see if there's enough bytesAvailable that the byteArray requires, but we don't count the quint32 'byte array size' that we peeked in that calculation
    {
