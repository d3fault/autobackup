...
public slots:
    void handleGetCouchbaseNotepadDocFinished(const QString &key, const QString &value);
    void handleAddCouchbaseNotepadDocFinished(const QString &key, const QString &value); //should i give them the value they just added? guess it depends on CAS stuff tbh, they might want to verify it's the same value etc
private slots:
