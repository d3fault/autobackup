...
void PaginationOfViewQuery::queryPageOfView(int pageNum_WithOneBeingTheFirstPage)
{
    ...
    std::string viewPath = "_design/dev_AllUsersWithAtLeastOneAdCampaign/_view/AllUsersWithAtLeastOneAdCampaign?stale=false&group_level=1&limit="; //once I upgrade to couchbase 3.0, I'll use stale=ok :-D
    viewPath += QString::number(itemsPerPage).toStdString();
