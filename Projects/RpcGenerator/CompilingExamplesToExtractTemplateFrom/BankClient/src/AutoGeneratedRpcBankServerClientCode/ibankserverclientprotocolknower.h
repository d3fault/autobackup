...
#endif
    QHash<uint /*HeaderMessageId*/, CreateBankAccountMessage*> m_PendingCreateBankAccountMessagesById;
    QHash<uint, GetAddFundsKeyMessage*> m_PendingGetAddFundsKeyMessagesById;
protected:
    ...
    CreateBankAccountMessage *getPendingCreateBankAccountMessageById(uint messageId);
    GetAddFundsKeyMessage *getPendingGetAddFundsKeyMessageById(uint messageId);
#if 0
...
public slots:
    ...
    //broadcast deliveries on the client are not needed, but we have them anyways because we are using shared code. they just recycle on the client.
