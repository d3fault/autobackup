...
//TODOmb: allow C++ editting directly on a class lifeline's "slot", rather than relying on Qt Creator for that. It would be a decent amount of work, but libclang would do assloads of the heavy lifting. Also, if such feature is implemented, it should be a checkbox in the toolbar to toggle whether or not C++ is shown (because the class lifelines would get so wide that we'd only be able to see one and it would defeat the purpose of the use case view). Other ideas could be zooming in/out (just in general), or even a zoom "level" that activates it the C++-on-class-lifeline-slot once you get close enough, or all of the above and customizeable as shit in the options menu
//TODOreq: holding down Ctrl or some other key while placing a new class in the class diagram tab places it then opens up the class editor (saving you from having to double click it). Or maybe it opens by default [UNLESS Ctrl is pressed?] TODOoptional: a similar thing could be done on the use case tab(s): holding Ctrl or whatever could open up the signal/slot message dialog between the "most recent item placed" (most likely class lifeline, but also actor) as the signal and the "item being drag-n-dropped" as the slot, saving you from having to draw a line
//TODOreq: New use Case -> place the actor for them. there are zero reasons to not have an actor in the use case, which means I should probably just remove it from the "items available to drop in use case", since I also never have 2 actors
//TODOmb: maybe a 'fast mode' checkbox, or maybe this is the default, idk just thinkin: when you set down a class lifeline in the use case, you're ALMOST ALWAYS going to then draw a signal from the item you set down last (actor or other class lifeline) to the new class lifeline, so that could be done automagically because it'd speed shit up
//TODOmb: default to "ok and make destination child of sender name m_bar0", rather than a toolbutton. OR at the very least it should be it's OWN button (below regular OK), because 1 click is < 2 clicks
//TODOmb: when creating a member/property, if the qualified type isn't "what it should be" (pointer-member for d=iQObjectClass, regular-stack-member for d=iImplicitlySharedDataType), then I should ask them if they want to JIT change it to what it should be. You almost never want a QObject child that's not on the heap (because of QObject parenting system), and similar'ish-but-not-very-much you almost never want an ImplicltlyShared data type on the heap because then you're not using it's much sought after copy semantecs and COW etc (hell I think it's even [increasing the chances that you're] breaking thread safety to use Implicitly Shared types on the heap (as a pointer) rather than on the stack with copy semantecs
