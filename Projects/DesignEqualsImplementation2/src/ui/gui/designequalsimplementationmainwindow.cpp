...
{
    connect(project, SIGNAL(typeAdded(DesignEqualsImplementationType*)), m_UseCaseUmlItemsWidget, SLOT(handleTypeAdded(DesignEqualsImplementationType*))); //Impulsively I feel the need to sever these connections when project tabs change, but since the backend project can't add a class when it's not current tab, there is little need (memory optimization? idfk)
