...
bool UserInterfaceSkeletonGenerator::generateRequestResponseContractGlueMaybe_AndAddContractSignalsAndSlotsToData(UserInterfaceSkeletonGeneratorData *data)
{
...
void UserInterfaceSkeletonGenerator::populateDataUsingHardCodedCppXD(UserInterfaceSkeletonGeneratorData &data)
{
    ...
    data.createAndAddSignal("error", UserInterfaceSkeletonGeneratorData::ArgsWithOptionalDefaultValues_List() << UserInterfaceSkeletonGeneratorData::SingleArgWithOptionalDefaultValue("QString","errorMessage")); //TO DOnvm: there will be an encodeVideoFinished(bool success) signal generated on business? wait not we PARSE the business, not generate it! so nvm actually, if you want an encodeVideoFinished signal, you have to specify it (you have to "check" it to choose it when we are parsing the class decl string [for the first time]

    data.createAndAddRequestResponse_aka_SlotWithFinishedSignal("queryInstalledCodecs", UserInterfaceSkeletonGeneratorData::ArgsList() << UserInterfaceSkeletonGeneratorData::SingleArg("QString","codecNameFilter"), UserInterfaceSkeletonGeneratorData::ArgsWithMandatoryDefaultValues_List() << UserInterfaceSkeletonGeneratorData::SingleArgWithMandatoryDefaultValue("QList<QString>","installedCodecs","QList<QString>()"));

    data.createAndAddRequestResponse_aka_SlotWithFinishedSignal("queryInstalledFormats", UserInterfaceSkeletonGeneratorData::ArgsList(), UserInterfaceSkeletonGeneratorData::ArgsWithMandatoryDefaultValues_List() << UserInterfaceSkeletonGeneratorData::SingleArgWithMandatoryDefaultValue("QList<QString>","installedFormats","QList<QString>()"));
}
...
void UserInterfaceSkeletonGenerator::generateUserInterfaceSkeletonFromData(const UserInterfaceSkeletonGeneratorData &data, QList<QString> implStubShortNames)
{
    ...
    UserInterfaceSkeletonGeneratorData dataCopy_Mutating = data; //we make a copy JIT and mutate it so that our signals/slots code gets generated by the usual routines
    if(!generateRequestResponseContractGlueMaybe_AndAddContractSignalsAndSlotsToData(&dataCopy_Mutating))
        return;

    displayFrontendBackendConnectStatements(dataCopy_Mutating);
