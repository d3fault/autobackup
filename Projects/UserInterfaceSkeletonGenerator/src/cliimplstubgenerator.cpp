...
void CliImplStubGenerator::generateImplHeaderAndSourceStubFiles(const UserInterfaceSkeletonGeneratorData &data, const QString &outputDirWithTrailingSlash)
{
    ...
    textStream << "class " << targetImplStubClassName(data.BusinessLogiClassName).toLower() << " : public QObject, public " << data.targetUserInterfaceClassName() << endl; //this class will have a lot in common with the gui stub generator, but the gui one will inherit QWidget instead of QObject
    textStream << "{" << endl;
    ...
    {
        textStream << tab << "void " << currentSlot.correspondingRequestSignalName() << currentSlot.argsWithParenthesis() << " override;" << endl;
    }
