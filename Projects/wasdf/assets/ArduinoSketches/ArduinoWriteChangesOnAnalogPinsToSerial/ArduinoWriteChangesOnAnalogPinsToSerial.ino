...
#define WASDF_JSON_KEY_STOPCOMMAND "stop"
#define WASDF_JSON_KEY_CONFIGURECOMMAND "configure"
#define WASDF_JSON_KEY_RESETCONFIGURATIONCOMMAND "resetConfigurations"
...
class AnalogPin_aka_FingerMb //TODOmb: shared declaration between PC and Arduino?
{
public:
    explicit AnalogPin_aka_FingerMb()
        : ChangeReportingSuppressedViaConfigurationCommand(false)
        , CurrentSensorValue(0)
        , OldSensorValue(0)
    { }
    bool changeReportingSuppressedViaConfigurationCommand() const
    {
        return ChangeReportingSuppressedViaConfigurationCommand;
    }
    void setChangeReportingSuppressedViaConfigurationCommand(bool changeReportingSuppressedViaConfigurationCommand)
    {
        ChangeReportingSuppressedViaConfigurationCommand = changeReportingSuppressedViaConfigurationCommand;
    }
    void configure(int analogPinId = -1, int atRestMin = -1, int atRestMax = -1, bool changeReportingSuppressedViaConfigurationCommand = false)
    {
    ...
    bool sanitizingAnalogRead(int *out_analogSensorValue)
    {
        if(IntPinId == -1)
            return false;
        int rawSensorValue = analogRead(IntPinId);
        if(rawSensorValue < MinAnalogPinValue || rawSensorValue > MaxAnalogPinValue)
            return false;
        *out_analogSensorValue = rawSensorValue;
        return true;
    }
    bool reportChangesInJsonObject(JsonObject *out_JsonObject)
    {
        if(fingerMoved())
        {
            (*out_JsonObject)[String(IntPinId)] = CurrentSensorValue;
            return true;
        }
        return false;
    }
    bool fingerMoved()
    {
         if(!sanitizingAnalogRead(&CurrentSensorValue))
             return false;
    ...
    bool ChangeReportingSuppressedViaConfigurationCommand;
    int IntPinId;
    ...
    int AtRestMax;
private:
    explicit AnalogPin_aka_FingerMb(const AnalogPin_aka_FingerMb &other) { } //delete
};
...
class AnalogPins_aka_FingersMb
{
public:
    explicit AnalogPins_aka_FingersMb()
    {
        resetConfigurations();
    }
    void configureAnalogPinAtIndex(int analogPinArrayIndex, int analogPinId, int atRestMin, int atRestMax, bool changeReportingSuppressedViaConfigurationCommand)
    {
        AllAnalogPins_aka_FingersMb[analogPinArrayIndex].configure(analogPinId, atRestMin, atRestMax, changeReportingSuppressedViaConfigurationCommand);
    }
    void resetConfigurations()
    {
        for(int i = 0; i < NUM_ANALOG_INPUTS; ++i)
        {
            AnalogPin_aka_FingerMb &currentAnalogPin = AllAnalogPins_aka_FingersMb[i];
            currentAnalogPin.configure(i);
        }
    }
    void reportChangesOverSerialPort()
    {
        MyJsonBuffer jsonBuffer; //TODOoptimization: jit/lazy create the buffer, only whenever myObject gets a value inserted
        JsonObject &myObject = jsonBuffer.createObject();
        ...
        {
            AnalogPin_aka_FingerMb &currentAnalogPin = AllAnalogPins_aka_FingersMb[i];
            if(currentAnalogPin.changeReportingSuppressedViaConfigurationCommand())
                continue;
            if(currentAnalogPin.reportChangesInJsonObject(&myObject))
                atLeastOneChanged = true;
        }

        if(!atLeastOneChanged) //TODOoptional: use myObject.keys().isEmpty
            return;
    ...
    void suppressAll()
    {
        ...
        {
            AnalogPin_aka_FingerMb &currentAnalogPin = AllAnalogPins_aka_FingersMb[i];
            currentAnalogPin.setChangeReportingSuppressedViaConfigurationCommand(true);
        }
    }
private:
    AnalogPin_aka_FingerMb AllAnalogPins_aka_FingersMb[NUM_ANALOG_INPUTS];
};

//Globals
bool Started;
AnalogPins_aka_FingersMb analogPins_aka_FingersMb;
...
void processInputCommandString(const String &inputCommandString)
{
    ...
    if(command == WASDF_JSON_KEY_STARTCOMMAND)
    {
        Started = true;
    }
    else if(command == WASDF_JSON_KEY_STOPCOMMAND)
    {
        Started = false;
    }
    else if(command == WASDF_JSON_KEY_CONFIGURECOMMAND)
    {
        //first set all analog pins to SUPPRESS their change reporting
        analogPins_aka_FingersMb.suppressAll();
        //then, using the received json config, re-enable the analog pins desired (this is done below, the 'false' param sent to configureAnalogPinAtIndex) -- this is easier than "figuring out which analog pins aren't desired" (and suppressing those only), and does the same thing
        ...
        while(it != atRestPositionsJsonObject.end())
        {
            ...
            analogPins_aka_FingersMb.configureAnalogPinAtIndex(i, analogPinId, atRestMin, atRestMax, false);
            ++it, ++i;
        }
        if(i < NUM_FINGERS)
        {
    ...
    else if(command == WASDF_JSON_KEY_RESETCONFIGURATIONCOMMAND)
    {
        analogPins_aka_FingersMb.resetConfigurations();
    }
    else
    {
...
{
    if(Started)
        analogPins_aka_FingersMb.reportChangesOverSerialPort(); //tell, don't ask ;-)
}
