...
//TODOmb: a 'profile' (like Firefox profile or Bitcoin datadir), which can load your "last used modules" (and hopefully their STATE!). if this is your first run or whatever you aren't calibrated, then the calibration module is loaded. whether or not the calibration module is loaded (and calibration window shown) is entirely up to you from then on: it's perfectly acceptable to keep it open 24/7/365, or to even change it's values while the app is running! in a sense the "calibration" module/widget could be a "raw finger positions" widget (but with other values useful for calibration). hell that same widget could be used whenever no arduino is connected, if the user wants to "synth" finger movements, I guess...
