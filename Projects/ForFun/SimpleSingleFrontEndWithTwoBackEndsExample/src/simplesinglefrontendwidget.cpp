...
SimpleSingleFrontendWidget::SimpleSingleFrontendWidget(QWidget *parent)
    : QWidget(parent)
{
...
void SimpleSingleFrontendWidget::setupGui()
{
    ...
    //Connect to our random number properties' changed() signals... so we can sanitize them and then send them to the backend
...
void SimpleSingleFrontendWidget::handleHashItButtonClicked()
{
...
void SimpleSingleFrontendWidget::checkTextActuallyChangedAndEmitIfTheyDid()
{
...
void SimpleSingleFrontendWidget::handleHashGenerated(const QString &hash)
{
    //This slot gets called whenever SimpleSingleBackend1::hashGenerated() is emitted
    m_HasherOutputPlainTextEdit->appendPlainText(hash);
}
void SimpleSingleFrontendWidget::handleRandomNumberResultsGathered(const QString &stringResult)
{
    //This slot gets called whenever SimpleSingleBackend2::randomNumberResultsGathered() is emitted
